...
def get_clean_sensors():
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # emp_names = [e['name'].lower() for e in employees] # Opcjonalne filtrowanie
...
def register_lovelace_resource():
    ...
    try:
        ...
        if get_resp.status_code != 200: return False, f"Błąd API: {get_resp.status_code}"
...
<head>
    ...
    <style>
        ...
        .sensor-tile:hover { background-color: #f1f3f5; }
...
<script>
    ...
    async function installCard() {
        if(!confirm("Dodać kartę do zasobów?")) return;
        try {
...
@app.route('/')
def index():
    # TU BYŁ BŁĄD - poprawione na get_clean_sensors()
